...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        try:
            icon_img = tk.Image("photo", file=self.THIS_FOLDER_G + "./files/pixxia.ico")
        ...
        self.file_entry_label1 = tk.Label(
            root, text="Enter Input Folder Path:", bg="#eeeeee", anchor=tk.W
        )
...
https://github.com/dhhruv
    ...
    def stop_callback(self):
        if not self.stopFlag:
            self.stopFlag = True
            self.stop = threading.Thread(
                target=self.stop_execute, name="Stopping_Thread", daemon=True
            )
            self.stop.start()
    ...
    def compress_callback(self):
        self.disable()
        self.compress = threading.Thread(
            target=self.compress_execute, name="Compression_Thread", daemon=True
        )
        self.compress.start()
    ...
    def compress_execute(self):
        try:
            ...
            if not self.raw_images:
                ...
                self.stopFlag = False
                self.enable()
                messagebox.showinfo("Pixxia", "Compression Completed !!")
        except Exception as e:
            messagebox.showinfo(
...
path_to_ico = os.path.abspath(os.path.join(bundle_dir, "./files/pixxia.ico"))
ROOT = tk.Tk()
